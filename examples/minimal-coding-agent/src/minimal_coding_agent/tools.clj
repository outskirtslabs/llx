...
(defn truncate-head
  "Keep first max-lines lines or max-bytes bytes, whichever limit is hit first.
   Returns a map with :content :truncated :total-lines :output-lines :truncated-by."
  [text]
...
(defn truncate-tail
  "Keep last max-lines lines or max-bytes bytes, whichever limit is hit first.
   Returns a map with :content :truncated :total-lines :output-lines :truncated-by."
  [text]
  (let [all-lines   (str/split-lines text)
        total-lines (count all-lines)]
...
(defn- fuzzy-find-text
  "Find old-text in content. Tries exact match first, then fuzzy match.
   Returns {:found bool :index int :match-length int :content-for-replacement string}."
  [content old-text]
...
(defn execute-write
  ...
  (let [f      (io/file path)
    ...
    (when (and parent (not (.exists parent)))
      (.mkdirs parent))
    (spit f content)
    (let [byte-count (count (.getBytes ^String content "UTF-8"))]
      {:content [{:type :text :text (str "Successfully wrote " byte-count " bytes to " path)}]})))
  ...
  (let [cwd       (System/getProperty "user.dir")
    ...
    (when-not (if timeout
      ...
      (let [partial (locking sb (str sb))
            msg     (str (when (seq partial) (str partial "\n\n"))
                         "Command timed out after " timeout " seconds")]
        (throw (ex-info msg {:type :timeout}))))
...
(defn make-tools []
  [{:name         "read"
    :description  (str "Read the contents of a file. Supports text files and images "
                       ...
                       (/ max-bytes 1024) "KB (whichever is hit first). "
   ...
   {:name         "bash"
    :description  (str "Execute a bash command in the current working directory. "
                       ...
                       max-lines " lines or " (/ max-bytes 1024) "KB "

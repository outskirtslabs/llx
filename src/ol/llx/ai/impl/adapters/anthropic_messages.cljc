(ns ol.llx.ai.impl.adapters.anthropic-messages
  (:require
...
(>defn build-request
       ([env model context opts]
        [:ol.llx/env :ol.llx/model :ol.llx/context-map :ol.llx/anthropic-provider-options => :ol.llx/adapter-request-map]
        (build-request env model context opts false))
       ([env model context opts stream?]
        [:ol.llx/env :ol.llx/model :ol.llx/context-map :ol.llx/anthropic-provider-options :boolean => :ol.llx/adapter-request-map]
           ...
           (let [index         (:index chunk)
             ...
             (case (get-in chunk [:delta :type])
               ...
               (let [delta        (get-in chunk [:delta :partial_json] "")
                     ...
                     parsed-args  (adapter-common/parse-json-safe env partial-json)
                     next-state   (-> state

(ns ol.llx.ai.impl.adapters.google-generative-ai
  (:require
...
(>defn build-request
       ([env model context opts]
        [:ol.llx/env :ol.llx/model :ol.llx/context-map :ol.llx/google-provider-options => :ol.llx/adapter-request-map]
        (build-request env model context opts false))
       ([env model context opts stream?]
        [:ol.llx/env :ol.llx/model :ol.llx/context-map :ol.llx/google-provider-options :boolean => :ol.llx/adapter-request-map]
        (let [api-key         (or (:api-key opts)
              ...
              base            (adapter-common/trim-trailing-slash (:base-url model))
              suffix          (if stream?

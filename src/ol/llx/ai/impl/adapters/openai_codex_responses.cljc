(ns ol.llx.ai.impl.adapters.openai-codex-responses
  (:require
   [clojure.string :as str]
   [com.fulcrologic.guardrails.malli.core :refer [>defn]]
   [ol.llx.ai.impl.adapters.openai-responses :as openai-responses]
   [ol.llx.ai.impl.oauth.openai-codex :as openai-codex]))
...
(>defn build-request
       ([env model context opts]
        [:ol.llx/env :ol.llx/model :ol.llx/context-map :ol.llx/openai-codex-responses-provider-options => :ol.llx/adapter-request-map]
        (build-request env model context opts false))
       ([env model context opts stream?]
        [:ol.llx/env :ol.llx/model :ol.llx/context-map :ol.llx/openai-codex-responses-provider-options :boolean => :ol.llx/adapter-request-map]

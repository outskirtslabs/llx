(ns ol.llx.ai.impl.adapters.openai-completions
  (:require
...
(>defn build-request
       ([env model context opts]
        [:ol.llx/env :ol.llx/model :ol.llx/context-map :ol.llx/openai-completions-provider-options => :ol.llx/adapter-request-map]
        (build-request env model context opts false))
       ([env model context opts stream?]
        [:ol.llx/env :ol.llx/model :ol.llx/context-map :ol.llx/openai-completions-provider-options :boolean => :ol.llx/adapter-request-map]
        (let [compat-profile   (resolve-compat model)

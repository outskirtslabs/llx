...
(>defn default-convert-to-llm
   ...
   whose `:role` is one of `:user`, `:assistant`, or `:tool-result`."
       [messages]
       [:llx.agent/messages => :llx/messages]
       (->> messages
            (filter (fn [message]
...
(defn- runtime-handle
  ...
  (schema/validate! schema-registry :llx.agent.loop/state state)
...
(defn rehydrate-agent
  "Creates an agent runtime from a previously persisted state snapshot.
   ...
   `state` must satisfy `:llx.agent.loop/state`.
...
(defn- dispatch-queued-messages!
  ...
  (let [messages (validate-with-agent-registry! agent :llx.agent/messages (or messages []))]

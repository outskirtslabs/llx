    ...
    (with-redefs [ai/default-env (fn [] ::ai-env)
      ...
      #?(:clj
         (let [env     (test-env
               ...
               out     (fx/execute-fx env {::fx/type :call-llm :messages [{:role :user :content "hi" :timestamp 2}]})
               signals (read-signals! out)]
...
(deftest execute-fx-validates-effect-shape-test
  #?(:clj
     (let [model (ai/get-model :openai "gpt-4o")
           env   (test-env {:llx.agent.loop/phase :llx.agent.loop/streaming
                            :system-prompt        "sys"
                            :model                model
                            :thinking-level       :off
                            :tools                []

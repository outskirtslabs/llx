...
(deftest model-utility-public-api-exists
  (is (ifn? (resolve-public 'llx-ai.models/get-model)))
  (is (ifn? (resolve-public 'llx-ai.models/get-models)))
  (is (ifn? (resolve-public 'llx-ai.models/get-providers)))
  (is (ifn? (resolve-public 'llx-ai.models/calculate-cost)))
  (is (ifn? (resolve-public 'llx-ai.models/supports-xhigh?)))
  (is (ifn? (resolve-public 'llx-ai.models/models-equal?))))

(deftest model-registry-lookup-contract
...
(deftest calculate-cost-contract
...
(deftest supports-xhigh-contract
  (let [supports-xhigh? (resolve-public 'llx-ai.models/supports-xhigh?)]
    (is (ifn? supports-xhigh?))
    (when (ifn? supports-xhigh?)
      (testing "anthropic opus 4.6 on anthropic-messages supports xhigh"
        (is (true? (supports-xhigh? {:id  "claude-opus-4.6"
                                     :api :anthropic-messages}))))
      (testing "same model family on non-anthropic-messages API does not support xhigh"
        (is (false? (supports-xhigh? {:id  "claude-opus-4.6"
                                      :api :openai-completions}))))
      (testing "non-matching models do not support xhigh"
        (is (false? (supports-xhigh? {:id  "gpt-4o-mini"
                                      :api :openai-completions})))))))

(deftest models-equal-contract

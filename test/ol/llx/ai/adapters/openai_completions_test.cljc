(ns ol.llx.ai.adapters.openai-completions-test
  (:require
   #?@(:clj [[clojure.test :refer [deftest is testing]]]
       :cljs [[cljs.test :refer-macros [deftest is testing]]])
   [clojure.string :as str]
   [ol.llx.ai.impl.adapters.openai-completions :as sut]
   [ol.llx.ai.impl.utils.unicode :as unicode]
   [ol.llx.ai.live.models :as live-models]
...
(defn- fixture
  [name]
  (util/read-edn-file (str "test/ol/llx/ai/fixtures/openai_completions/" name ".edn")))
  ...
  (util/with-captured-logs!
    (fn [logs*]
      (let [context {:messages [{:role :user :content "hello" :timestamp 1}]}
            ...
            payload (util/json-read (:body request) {:key-fn keyword})
            event   (util/first-event logs* :ol.llx.obs/provider-payload)]
        (is (util/submap?

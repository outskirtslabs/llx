(ns ol.llx.ai.client.jvm-test
  (:require
   [babashka.json :as json]
   [clojure.test :refer [deftest is testing]]
   [ol.llx.ai.impl.client.jvm :as sut]
  ...
  (util/with-captured-logs!
    (fn [logs*]
      (let [line    (str "data: " (json/write-str {:type "delta" :text "ok"}) "\n" "data: [DONE]\n")
        ...
        (let [start (util/first-event logs* :ol.llx.obs/stream-start)
          ...
          (is (util/submap?
               {:id   :ol.llx.obs/stream-item-received                                                                     :level :trace
                :data {:call-id    "call_1" :provider       :openai     :api   :openai-completions :model-id "gpt-4o-mini"
    ...
    (fn [logs*]
      (let [out     (sp/chan)
        ...
        (sut/run-stream! {:adapter      adapter
                          ...
                          :request-opts {}})
        (collect-stream! out 1000)
        (let [event (util/first-event logs* :ol.llx.obs/stream-event-error)]
          (is (util/submap?
               {:id    :ol.llx.obs/stream-event-error
                ...
                :data  {:call-id                 "call_2"
                        ...
                        :error-type              :ol.llx/streaming-error

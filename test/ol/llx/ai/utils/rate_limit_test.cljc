(ns ol.llx.ai.utils.rate-limit-test
  (:require
   #?@(:clj [[clojure.test :refer [deftest is testing]]]
       :cljs [[cljs.test :refer-macros [deftest is testing]]])
   [ol.llx.ai.impl.utils.rate-limit :as sut]))

(deftest rate-limited-detection
  (testing "detects structured error types"
    (is (true? (sut/rate-limited? {:stop-reason :error :error-type :ol.llx/rate-limit})))
    (is (true? (sut/rate-limited? {:stop-reason :error :error-type :ol.llx/quota-exceeded}))))

(ns ol.llx.ai.model-catalog.generate-test
  (:require
   ...
   [ol.llx.ai.impl.model-catalog.generate :as generate]
...
(defn- fixture-path [name]
  (str "test/ol/llx/ai/fixtures/model_catalog/" name))
...
(deftest build-catalog-normalizes-supported-providers-only
  (let [build-catalog (resolve-public 'ol.llx.ai.impl.model-catalog.generate/build-catalog)]
    ...
    (when (ifn? build-catalog)
      (let [catalog (build-catalog {:models-dev-data (load-models-dev-fixture)
        ...
        (doseq [model catalog]
          (is (schema/valid? :ol.llx/model model)))))))
...
(deftest render-generated-source-is-stable
  (let [build-catalog (resolve-public 'ol.llx.ai.impl.model-catalog.generate/build-catalog)
        render-source (resolve-public 'ol.llx.ai.impl.model-catalog.generate/render-generated-source)]
    (is (ifn? build-catalog))
    ...
    (when (and (ifn? build-catalog)
      ...
      (let [catalog  (build-catalog {:models-dev-data (load-models-dev-fixture)
        ...
        (is (str/includes? rendered "(ns ol.llx.ai.impl.models-generated"))))))

(ns ol.llx.ai.client-test
  (:require
   #?@(:clj [[clojure.test :refer [deftest is]]
             [ol.llx.ai.impl.client.jvm :as runtime]]
       :cljs [[cljs.test :refer-macros [deftest is]]
              [ol.llx.ai.impl.client.node :as runtime]])
   #?@(:clj [[ol.llx.ai.test-util :as util]]
       :cljs [[ol.llx.ai.test-util :as util :include-macros true]])
   [clojure.string :as str]
   [ol.llx.ai.impl.client :as sut]
...
(defn- rejects-with!
  ...
  (-> deferred
      (p/then (fn [_]
                (throw (ex-info "Expected rejection" {:type :ol.llx/test-expected-rejection}))))
      (p/catch (fn [ex]
                 (assert-ex! ex)
                 true))
      (p/then (fn [_]
                    ...
                    (fn [logs*]
                      (let [env     (stub-env (fn [_request]
                        ...
                        (-> (sut/complete* env base-model context {:api-key "x"})
                            (p/then (fn [_]
                                      (let [start      (util/first-event logs* :ol.llx.obs/call-start)
                      ...
                      (let [env     (stub-env (fn [_request]
                        ...
                        (-> (sut/complete* env base-model context {:api-key "bad"})
                            (p/then (fn [_]
                                      (throw (ex-info "Expected rejection" {:type :ol.llx/test-expected-rejection}))))
                            (p/catch (fn [_]
                                       (let [status (util/first-event logs* :ol.llx.obs/http-status-error)
                                         ...
                                         (is (util/submap?
                                              {:id    :ol.llx.obs/call-error
                                               :level :error
                                               :data  {:operation  :complete
                                                       :error-type :ol.llx/authentication-error}}
...
(deftest complete-rejects-unsupported-xhigh-with-structured-error
  (util/async done
              (let [env     (stub-env (fn [_request]
                ...
                (rejects-with! d
                               (fn [ex]
                                 (is (= :ol.llx/unsupported-reasoning-level (-> ex ex-data :type)))

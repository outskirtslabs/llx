...
(defn- suite->build
  ...
  (case (suite-id suite)
    :cljs-live {:build-id   :kaocha-test-live
                :cache-dir  ".shadow-cljs/builds/kaocha-test-live"
                :output-to  "target/kaocha-tests-live.js"
                :output-dir "target/kaocha-test-live"}
    {:build-id   :kaocha-test
     :cache-dir  ".shadow-cljs/builds/kaocha-test"
     :output-to  "target/kaocha-tests.js"
     :output-dir "target/kaocha-test"}))
...
(defn- start-node-client!
  ...
  (try
    ...
    (catch Exception _))
  (stop-node-client!)
  (let [{:keys [build-id cache-dir output-dir output-to]} (suite->build suite)
        _                                                 (shell/sh "rm" "-rf" cache-dir output-dir output-to)

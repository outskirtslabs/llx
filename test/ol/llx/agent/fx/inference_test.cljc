(ns ol.llx.agent.fx.inference-test
  (:require
   #?@(:clj [[clojure.test :refer [deftest is]]]
       :cljs [[cljs.test :refer-macros [deftest is]]])
   [ol.llx.agent.fx.inference :as sut]
   [ol.llx.ai :as ai]))
...
(deftest llm-event->step-start-initializes-partial-test
  (let [model                          (ai/get-model :openai "gpt-4o")
    ...
    (is (= [] (:content partial)))
    (is (= :ol.llx.agent.signal/llm-start (:type signal)))
...
(deftest llm-event->step-text-delta-updates-chunk-test
  (let [model             (ai/get-model :openai "gpt-4o")
    ...
    (is (false? (:done? step)))
    (is (= :ol.llx.agent.signal/llm-chunk (get-in step [:signal :type])))
...
(deftest llm-event->step-done-emits-assistant-message-test
  (let [model   (ai/get-model :openai "gpt-4o")
    ...
    (is (true? (:done? step)))
    (is (= :ol.llx.agent.signal/llm-done (get-in step [:signal :type])))

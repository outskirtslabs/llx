...
(defn- assert-execute-tool-behaviors!
  ...
  (is (= [:ol.llx.agent.signal/tool-update
  ...
  (is (= [:ol.llx.agent.signal/tool-error] (mapv :type missing-signals)))
  (is (= :ol.llx/tool-not-found
  ...
  (is (= [:ol.llx.agent.signal/tool-error] (mapv :type validation-signals)))
  (is (= :ol.llx/validation-error
...
(deftest execute-fx-validates-effect-shape-test
  #?(:clj
     (let [model (ai/get-model :openai "gpt-4o")
       ...
       (is (thrown? Exception
                    (fx/execute-fx env {:ol.llx.agent.fx/type :call-llm})))
       (is (thrown? Exception
                    (fx/execute-fx env {:ol.llx.agent.fx/type :unknown})))
       (is (thrown? Exception
                    (fx/execute-fx env
                                   {:ol.llx.agent.fx/type :emit-event
                                    :event                {:type :ol.llx.agent.event/message-update}})))

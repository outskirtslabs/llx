(ns ol.llx.agent-test
  (:require
   ...
   [ol.llx.agent.schema :as agent.schema]
...
(deftest rehydrate-agent-uses-snapshot-test
  (let [model (ai/get-model :openai "gpt-4o")
        ...
        state {:ol.llx.agent.loop/phase :ol.llx.agent.loop/idle
...
(deftest subscribe-unsubscribe-test
  (let [agent      (sut/create-agent required-env-opts)
        ...
        event-1    {:type :ol.llx.agent.event/test :n 1}
        event-2    {:type :ol.llx.agent.event/test :n 2}]
...
(deftest command-wrappers-dispatch-through-driver-run-test
  (let [seen*  (atom [])
    ...
    (with-redefs [driver/run (fn [env input]
      ...
      (is (= [{:type     :ol.llx.agent.command/prompt
               :messages [{:role :user :content "hello" :timestamp 1}]}
              {:type :ol.llx.agent.command/continue}
              {:type :ol.llx.agent.command/abort}
              {:type :ol.llx.agent.command/set-model :model model}
              {:type :ol.llx.agent.command/set-system-prompt :system-prompt "new prompt"}]
             (mapv second @seen*))))))
...
(deftest set-model-validates-model-shape-test
  (let [seen*       (atom [])
    ...
    (with-redefs [driver/run (fn [_env input]
      ...
      (is (= [{:type :ol.llx.agent.command/set-model :model valid-model}]
             @seen*)))))
...
(deftest custom-message-schemas-validate-message-and-messages-test
  (let [custom-message          {:role      :my-app/notification
        ...
        custom-enabled-registry (mr/composite-registry
                                 ...
                                 {:my/message-notification
                                  [:map
                                   ...
                                   [:text :string]
                                   [:timestamp :int]]})]
    (testing ":ol.llx.agent/custom-message-schemas requires namespace-qualified dispatch keys"
      (is (thrown? #?(:clj clojure.lang.ExceptionInfo
                      :cljs js/Error)
    ...
    (testing ":ol.llx.agent/message rejects custom roles when not registered"
      (is (thrown? #?(:clj clojure.lang.ExceptionInfo
                      :cljs js/Error)
                   (agent.schema/validate! base-registry :ol.llx.agent/message custom-message))))

    (testing ":ol.llx.agent/message accepts registered custom role"
      (is (= custom-message
             (agent.schema/validate! custom-enabled-registry :ol.llx.agent/message custom-message))))

    (testing ":ol.llx.agent/messages accepts canonical + registered custom messages"
    ...
    (testing ":ol.llx.agent/messages rejects invalid registered custom message payloads"
      (is (thrown? #?(:clj clojure.lang.ExceptionInfo
                      :cljs js/Error)
                   (agent.schema/validate! custom-enabled-registry
                                           :ol.llx.agent/messages
...
(deftest custom-message-public-api-validation-test
  (let [custom-message-schemas {:my-app/notification :my/message-notification}
    ...
    (is (= [{:type :ol.llx.agent.command/prompt :messages [custom-message]}
            {:type :ol.llx.agent.command/append-message :message custom-message}
...
(deftest custom-message-schema-may-reference-provided-schemas-test
  (let [custom-message-schemas {:my-app/rich-note :my/rich-note-message}
    ...
    (is (= [{:type :ol.llx.agent.command/prompt :messages [valid-message]}]
           @seen*))))
...
(deftest tool-call-schema-contract-test
  (let [schema-registry (agent.schema/registry {})
    ...
    (is (= tool-call
           (agent.schema/validate! schema-registry :ol.llx.agent/tool-call tool-call)))
...
(deftest tool-schema-tightening-on-commands-and-state-test
  (let [schema-registry      (agent.schema/registry {})
        ...
        set-tools-command    {:type  :ol.llx.agent.command/set-tools
                              :tools [tool]}
        invalid-set-tools    {:type  :ol.llx.agent.command/set-tools
                              :tools [{:name "read_file"}]}
    ...
    (testing ":ol.llx.agent/command-set-tools expects :ol.llx.agent/tool entries"
      (is (= set-tools-command
             (agent.schema/validate! schema-registry :ol.llx.agent/command-set-tools set-tools-command)))
      (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry
                                                                :ol.llx.agent/command-set-tools
                                                                invalid-set-tools))))
    (testing ":ol.llx.agent/create-agent-opts expects :ol.llx.agent/tool entries in :tools"
      (is (= valid-create-agent
             (agent.schema/validate! schema-registry :ol.llx.agent/create-agent-opts valid-create-agent)))
      (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry
                                                                :ol.llx.agent/create-agent-opts
                                                                invalid-create-agent))))
    (testing ":ol.llx.agent.loop/state expects :ol.llx.agent/tool entries in :tools"
      (is (= loop-state
             (agent.schema/validate! schema-registry :ol.llx.agent.loop/state loop-state)))
      (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry
...
(deftest effect-execute-tool-tool-call-schema-test
  (let [schema-registry (agent.schema/registry {})
        effect          {:ol.llx.agent.fx/type :execute-tool
                         :tool-call            (tool-schema-valid-tool-call)}
        invalid-effect  {:ol.llx.agent.fx/type :execute-tool
                         :tool-call            {:id "tc-1"}}]
    (is (= effect
           (agent.schema/validate! schema-registry :ol.llx.agent.fx/effect-execute-tool effect)))
    (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry
...
(deftest tool-and-pending-tool-call-schema-test
  (let [schema-registry       (agent.schema/registry {})
    ...
    (is (= tool
           (agent.schema/validate! schema-registry :ol.llx.agent/tool tool)))
    (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry :ol.llx.agent/tool invalid-tool)))
    (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry
                                                              :ol.llx.agent.loop/state
...
(deftest tool-signal-and-event-schema-tightening-test
  (let [schema-registry       (agent.schema/registry {})
        ...
        result-signal         {:type                :ol.llx.agent.signal/tool-result
        ...
        invalid-result-signal (assoc result-signal :result {:role         :tool-result
                                                            :tool-call-id "tc-1"})
        error-signal          {:type                :ol.llx.agent.signal/tool-error
                               :tool-call-id        "tc-1"
                               :error               {:type :ol.llx/tool-not-found}
                               :tool-result-message canonical-tool-result}
        invalid-error-signal  (assoc error-signal :tool-result-message {:role :tool-result})
        execution-end-event   {:type         :ol.llx.agent.event/tool-execution-end
                               :tool-call-id "tc-1"
                               :tool-name    "read_file"
                               :result       canonical-tool-result
                               :is-error?    false}
        invalid-execution-end {:type   :ol.llx.agent.event/tool-execution-end
                               :result canonical-tool-result}]
    (is (= result-signal
           (agent.schema/validate! schema-registry :ol.llx.agent/signal-tool-result result-signal)))
    (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry
                                                              :ol.llx.agent/signal-tool-result
                                                              invalid-result-signal)))
    (is (= error-signal
           (agent.schema/validate! schema-registry :ol.llx.agent/signal-tool-error error-signal)))
    (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry
                                                              :ol.llx.agent/signal-tool-error
                                                              invalid-error-signal)))
    (is (= execution-end-event
           (agent.schema/validate! schema-registry :ol.llx.agent/event-tool-execution-end execution-end-event)))
    (is (tool-schema-ex-info-thrown? #(agent.schema/validate! schema-registry
